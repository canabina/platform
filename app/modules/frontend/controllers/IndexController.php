...
class IndexController extends Controller
{
	...
	public function afterAction(){
		...
		$session = Sili::$app->session;

		$request = Sili::$app->request;

		$cityName = $request->get('cityName');

		if (!$session->get('lastVisitedCity') && $cityName || $cityName && $session->get('lastVisitedCity') != $cityName )
			$session->set('lastVisitedCity', $cityName);
	...
	public function isError($code = false, $isAjax = false){
