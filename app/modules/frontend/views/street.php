...
<div class="page-body street" >	
	<h2>
		Район: <b><?= Sili::$app->html->encode($area['new_name']) ?> (<?= Sili::$app->html->encode($area['old_name']) ?>)</b>
	</h2>
	<h3>
		<?= Sili::$app->html->encode($street['object_type']) ?>
	</h3>
	...
	<p>
		...
		<b>
			<?= Sili::$app->html->encode($street['new_name']) ?>
		</b>
	</p>
	<p>
		<label>
		...
		<b>
			<?= Sili::$app->html->encode($street['old_name']) ?>
		</b>
	</p>

	<p>
		<b>
			<?= Sili::$app->html->encode($street['date_and_number_of_resolve']) ?>
		</b>
	</p>
             ...
             <?php 
                ...
                foreach (['', 'Особа', 'Об’єкт', 'Історична назва', 'Суб’єкт'] as $key => $value) 
                   if ($street['eponim_type'] == $key) 
                      $res = ($street['eponim'] && $street['eponim'] != 'історична назва') ? '<a href="'.Sili::$app->html->encode($street['eponim']).'" target="__blank">Названо на честь</a>' : Sili::$app->html->encode($value);
                echo $res;
              ?>
          <?php endif; ?>
	</p>
	...
	<input type="hidden" value='Кіровоград, <?= Sili::$app->html->encode($street['old_name']) ?>' id='address'>
</div>
